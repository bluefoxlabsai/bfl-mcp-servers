...
# Microsoft Teams MCP Helm Chart Installation Script
#
# Usage:
#   ./install.sh [release-name] [options]
#
# Environment Variables:
#   AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, AZURE_TENANT_ID - Your Azure AD credentials (optional, will prompt if not set)
...
#   ./install.sh my-teams-mcp                 # Install with custom release name
#   AZURE_CLIENT_ID=xyz ./install.sh          # Install with pre-set client ID
#   NAMESPACE=teams ./install.sh              # Install in 'teams' namespace
...
print_color $BLUE "🚀 Microsoft Teams MCP Helm Chart Installer"
echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, AZURE_TENANT_ID - Your Azure AD credentials"
...
# Get Azure AD credentials from user if not provided via environment
...
if [ -z "$AZURE_CLIENT_ID" ] || [ -z "$AZURE_CLIENT_SECRET" ] || [ -z "$AZURE_TENANT_ID" ]; then
    print_color $RED "❌ All Azure AD credentials are required. Please set environment variables or enter them when prompted."
    print_color $RED "   Required: AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, AZURE_TENANT_ID"
    exit 1
fi
...
if [ "$DRY_RUN" = true ]; then
    print_color $YELLOW "🔧 Dry run - templating Microsoft Teams MCP..."
    helm template "$RELEASE_NAME" "$CHART_PATH" "${HELM_ARGS[@]}"
elif [ "$UPGRADE" = true ]; then
    print_color $YELLOW "🔧 Upgrading Microsoft Teams MCP..."
    helm upgrade "$RELEASE_NAME" "$CHART_PATH" "${HELM_ARGS[@]}"
else
    print_color $YELLOW "🔧 Installing Microsoft Teams MCP..."
...
if [ $? -eq 0 ] && [ "$DRY_RUN" = false ]; then
    ...
    if [ "$TRANSPORT" = "stdio" ]; then
        ...
        echo "export POD_NAME=\$(kubectl get pods --namespace $NAMESPACE -l \"app.kubernetes.io/name=teams-mcp,app.kubernetes.io/instance=$RELEASE_NAME\" -o jsonpath=\"{.items[0].metadata.name}\")"
        echo "kubectl exec -it \$POD_NAME --namespace $NAMESPACE -- python mcp_teams/server.py"
    else
        print_color $BLUE "📋 For HTTP transport ($TRANSPORT):"
        echo "kubectl --namespace $NAMESPACE port-forward service/$RELEASE_NAME-teams-mcp 8080:8000"
        ...
        echo "    url: \"http://$RELEASE_NAME-teams-mcp.$NAMESPACE.svc.cluster.local:8000\""
        echo "    timeout: 30000"
        echo ""
    ...
    echo "kubectl get pods --namespace $NAMESPACE -l app.kubernetes.io/name=teams-mcp"
    echo "kubectl logs --namespace $NAMESPACE -l app.kubernetes.io/name=teams-mcp -f"
    echo ""
    ...
    echo "3. Check logs: kubectl logs -n $NAMESPACE -l app.kubernetes.io/name=teams-mcp"
    exit 1
fi