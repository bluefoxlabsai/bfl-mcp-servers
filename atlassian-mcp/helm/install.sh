...
# Atlassian MCP Helm Chart Installation Script
#
# Usage:
#   ./install.sh [release-name] [options]
#
# Environment Variables:
#   CONFLUENCE_URL, CONFLUENCE_USERNAME, CONFLUENCE_API_TOKEN, JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN - Your Atlassian credentials (optional, will prompt if not set)
...
#   ./install.sh my-atlassian-mcp                # Install with custom release name
...
print_color $BLUE "🚀 Atlassian MCP Helm Chart Installer"
echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  CONFLUENCE_URL, CONFLUENCE_USERNAME, CONFLUENCE_API_TOKEN, JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN - Your Atlassian credentials"
...
# Get Atlassian credentials from user if not provided via environment
if [ -z "$CONFLUENCE_URL" ]; then
    echo ""
    print_color $YELLOW "📋 Please enter your Confluence URL (e.g., https://mycompany.atlassian.net/wiki):"
    read CONFLUENCE_URL
fi

if [ -z "$CONFLUENCE_USERNAME" ]; then
    echo ""
    print_color $YELLOW "📋 Please enter your Confluence username/email:"
    read CONFLUENCE_USERNAME
fi

if [ -z "$CONFLUENCE_API_TOKEN" ]; then
    echo ""
    print_color $YELLOW "📋 Please enter your Confluence API token:"
...
if [ -z "$JIRA_URL" ]; then
    echo ""
    print_color $YELLOW "📋 Please enter your Jira URL (e.g., https://mycompany.atlassian.net):"
    read JIRA_URL
fi

if [ -z "$JIRA_USERNAME" ]; then
    echo ""
    print_color $YELLOW "📋 Please enter your Jira username/email:"
    read JIRA_USERNAME
fi

if [ -z "$JIRA_API_TOKEN" ]; then
    echo ""
    print_color $YELLOW "📋 Please enter your Jira API token:"
    read -s JIRA_API_TOKEN
    echo ""
fi

# Validate required credentials
if [ -z "$CONFLUENCE_URL" ] || [ -z "$CONFLUENCE_USERNAME" ] || [ -z "$CONFLUENCE_API_TOKEN" ] || 
   [ -z "$JIRA_URL" ] || [ -z "$JIRA_USERNAME" ] || [ -z "$JIRA_API_TOKEN" ]; then
    print_color $RED "❌ All Atlassian credentials are required. Please set environment variables or enter them when prompted."
    print_color $RED "   Required: CONFLUENCE_URL, CONFLUENCE_USERNAME, CONFLUENCE_API_TOKEN, JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN"
    exit 1
fi
...
HELM_ARGS=(
    --namespace "$NAMESPACE"
    --set secret.confluenceUrl="$CONFLUENCE_URL"
    --set secret.confluenceUsername="$CONFLUENCE_USERNAME"
    --set secret.confluenceApiToken="$CONFLUENCE_API_TOKEN"
    --set secret.jiraUrl="$JIRA_URL"
    --set secret.jiraUsername="$JIRA_USERNAME"
    --set secret.jiraApiToken="$JIRA_API_TOKEN"
...
if [ "$DRY_RUN" = true ]; then
    print_color $YELLOW "🔧 Dry run - templating Atlassian MCP..."
    helm template "$RELEASE_NAME" "$CHART_PATH" "${HELM_ARGS[@]}"
elif [ "$UPGRADE" = true ]; then
    print_color $YELLOW "🔧 Upgrading Atlassian MCP..."
    helm upgrade "$RELEASE_NAME" "$CHART_PATH" "${HELM_ARGS[@]}"
else
    print_color $YELLOW "🔧 Installing Atlassian MCP..."
...
if [ $? -eq 0 ] && [ "$DRY_RUN" = false ]; then
    ...
    if [ "$TRANSPORT" = "stdio" ]; then
        ...
        echo "export POD_NAME=\$(kubectl get pods --namespace $NAMESPACE -l \"app.kubernetes.io/name=atlassian-mcp,app.kubernetes.io/instance=$RELEASE_NAME\" -o jsonpath=\"{.items[0].metadata.name}\")"
        ...
        echo "kubectl --namespace $NAMESPACE port-forward service/$RELEASE_NAME-atlassian-mcp 8080:8000"
        ...
        echo "    url: \"http://$RELEASE_NAME-atlassian-mcp.$NAMESPACE.svc.cluster.local:8000\""
        echo "    timeout: 30000"
        echo ""
    ...
    echo "kubectl get pods --namespace $NAMESPACE -l app.kubernetes.io/name=atlassian-mcp"
    echo "kubectl logs --namespace $NAMESPACE -l app.kubernetes.io/name=atlassian-mcp -f"
    echo ""
    ...
    echo "3. Check logs: kubectl logs -n $NAMESPACE -l app.kubernetes.io/name=atlassian-mcp"
    exit 1
fi