...
# SharePoint MCP Helm Chart Uninstallation Script
#
# Usage:
...
#   ./uninstall.sh my-sharepoint-mcp  # Uninstall specific release
#   ./uninstall.sh --dry-run          # See what would be deleted
#   ./uninstall.sh --keep-namespace   # Don't delete the namespace
#   NAMESPACE=sharepoint ./uninstall.sh sharepoint-mcp  # Non-interactive
...
print_color $BLUE "🗑️  SharePoint MCP Helm Chart Uninstaller"
echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  $0 my-sharepoint-mcp  # Uninstall specific release"
...
# Confirm deletion
echo ""
print_color $YELLOW "⚠️  This will permanently delete the SharePoint MCP server deployment."
...
print_color $YELLOW "🔧 Uninstalling SharePoint MCP..."
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    print_color $GREEN "🎉 SharePoint MCP has been successfully uninstalled!"
    echo ""
    print_color $BLUE "📋 To verify removal:"
    echo "kubectl get pods -n $NAMESPACE -l app.kubernetes.io/name=sharepoint-mcp"
